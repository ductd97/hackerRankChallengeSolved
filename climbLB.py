def removeDup(ranked):
    result = [ranked[0]]
    for i in range(1,len(ranked)):
        if ranked[i]!=result[-1]:
            result.append(ranked[i])
    return result


def climbLB(ranked, player):
    result = []
    r = removeDup(ranked)
        ...
        while len(r):
            ...
            if player[i] > r[-1]:
                ...
                if len(r) < 1:
                    result.append(1)
                    break
# Fail time execute with large dataset
# def climbLB(ranked, player):
#     r = removeDup(ranked)
#     rank = len(r)
#     for i in range(len(player)):
...
def main():
    ...
    ranked = [int(i) for i in ranked.split(" ")]
    ...
    player = [int(i) for i in player.split(" ")]
    climbLB(ranked, player)
